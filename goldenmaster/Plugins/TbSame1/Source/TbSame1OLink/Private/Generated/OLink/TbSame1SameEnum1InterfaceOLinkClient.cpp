...
void UTbSame1SameEnum1InterfaceOLinkClient::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	TWeakObjectPtr<UTbSame1SameEnum1InterfaceOLinkClient> weak(this);
...
TFuture<ETbSame1Enum1> UTbSame1SameEnum1InterfaceOLinkClient::Func1Async(ETbSame1Enum1 Param1)
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("ApiGear.TbSame1.SameEnum1Interface.OLink.Func1Async");
	if (!m_sink->IsReady())
	{
		UE_LOG(LogTbSame1SameEnum1InterfaceOLinkClient, Error, TEXT("%s has no node. Probably no valid connection or service. Are the ApiGear TbSame1 plugin settings correct? Service set up correctly?"), UTF8_TO_TCHAR(m_sink->olinkObjectName().c_str()));
	...
	m_sink->GetNode()->invokeRemote(memberId, {Param1},
		...
		{
			UE_LOG(LogTbSame1SameEnum1InterfaceOLinkClient, Error, TEXT("Func1Async: invalid return value type or null -> returning default"));
			Promise->SetValue(ETbSame1Enum1::TS1E1_Value1);
		}
	});
...
void UTbSame1SameEnum1InterfaceOLinkClient::emitSignal(const std::string& signalName, const nlohmann::json& args)
{
	...
	if (signalName == "sig1")
	{
		...
		{
			UE_LOG(LogTbSame1SameEnum1InterfaceOLinkClient, Error, TEXT("Signal sig1: invalid args array (expected 1 elements)"));
			return;
		}
		// make sure the type matches our expectation
		if (args[0].is_null() || !args[0].is_number_integer())
		{
			UE_LOG(LogTbSame1SameEnum1InterfaceOLinkClient, Error, TEXT("Signal param1: invalid type for parameter 0"));
			return;
		}
