...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("ApiGear.TbSame1.SameStruct2Interface.OLink.ServerInvoke");
	if (!BackendService)
	{
	...
	if (path == "func1")
	{
		...
		{
			UE_LOG(LogTbSame1SameStruct2InterfaceOLinkSource, Error, TEXT("olinkInvoke: 'func1' expects 1 arg(s), got %d"), args.is_array() ? static_cast<int>(args.size()) : -1);
			return nlohmann::json();
		}
	...
	if (path == "func2")
	{
		...
		{
			UE_LOG(LogTbSame1SameStruct2InterfaceOLinkSource, Error, TEXT("olinkInvoke: 'func2' expects 2 arg(s), got %d"), args.is_array() ? static_cast<int>(args.size()) : -1);
			return nlohmann::json();
		}
		FTbSame1Struct1 Param1 = args.at(0).get<FTbSame1Struct1>();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("ApiGear.TbSame1.SameStruct2Interface.OLink.ServerSetProperty");
	if (!BackendService)
	{
		UE_LOG(LogTbSame1SameStruct2InterfaceOLinkSource, Error, TEXT("No backend service set - please specify a service in the adapter TbSame1SameStruct2InterfaceOLinkAdapter which implements the TbSame1SameStruct2Interface interface"));
		return;
