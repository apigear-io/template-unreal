...
#include "Generated/Detail/TbStructArrayPromiseHolder.h"

DECLARE_LOG_CATEGORY_EXTERN(LogTbStructArrayMethodHelper_JNI, Log, All);

class FTbStructArrayMethodHelper
{
public:
	explicit FTbStructArrayMethodHelper(const FString& InOwnerName)
	...
	{
		TMap<FGuid, TUniquePtr<ITbStructArrayPromiseHolder>> ReplyPromisesMapCopy;
		{
	...
	{
		UE_LOG(
			LogTbStructArrayMethodHelper_JNI,
			Verbose,
			TEXT("Resolving promise %s with default value for %s"),
			*(InId.ToString(EGuidFormats::Digits)),
			*OwnerName);

		TUniquePtr<ITbStructArrayPromiseHolder> PromiseHolderPtr;
		{
			FScopeLock Lock(&ReplyPromisesMapCS);
...
private:
	TMap<FGuid, TUniquePtr<ITbStructArrayPromiseHolder>> ReplyPromisesMap;
	FCriticalSection ReplyPromisesMapCS;
	FString OwnerName;
};

template <typename ResultType>
FGuid FTbStructArrayMethodHelper::StorePromise(TPromise<ResultType>&& Promise)
{
...

	{
		...
		ReplyPromisesMap.Add(
			Id, MakeUnique<TTbStructArrayPromiseHolder<ResultType>>(MoveTemp(Promise)));
	}
...
template <typename ResultType>
bool FTbStructArrayMethodHelper::FulfillPromise(const FGuid& Id, const ResultType& Value)
{
	UE_LOG(
		LogTbStructArrayMethodHelper_JNI,
		Verbose,
	...
	TUniquePtr<TTbStructArrayPromiseHolder<ResultType>> PromiseHolderPtr;

	{
		...
		if (auto* Found = ReplyPromisesMap.Find(Id))
		{
			PromiseHolderPtr.Reset(
				static_cast<TTbStructArrayPromiseHolder<ResultType>*>(Found->Release()));
