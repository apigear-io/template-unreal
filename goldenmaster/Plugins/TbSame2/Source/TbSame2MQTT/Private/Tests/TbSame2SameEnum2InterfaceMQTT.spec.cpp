...
namespace Tests
{
BEGIN_DEFINE_SPEC(UTbSame2SameEnum2InterfaceMQTTSpec, "TbSame2.SameEnum2Interface.MQTT", TbSame2TestFilterMask);

TUniquePtr<FTbSame2SameEnum2InterfaceMQTTFixture> ImplFixture;

END_DEFINE_SPEC(UTbSame2SameEnum2InterfaceMQTTSpec);

void UTbSame2SameEnum2InterfaceMQTTSpec::Define()
{
	LatentBeforeEach([this](const FDoneDelegate& TestDone)
		{
		ImplFixture = MakeUnique<FTbSame2SameEnum2InterfaceMQTTFixture>();
		TestTrue("Check for valid ImplFixture", ImplFixture.IsValid());

		TestTrue("Check for valid testImplementation", ImplFixture->GetImplementation().GetInterface() != nullptr);

		// setup client
		UTbSame2SameEnum2InterfaceMQTTClient* MQTTClient = Cast<UTbSame2SameEnum2InterfaceMQTTClient>(ImplFixture->GetImplementation().GetObject());
