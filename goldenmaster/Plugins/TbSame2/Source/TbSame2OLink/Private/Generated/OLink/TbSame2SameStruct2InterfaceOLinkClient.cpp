...
void UTbSame2SameStruct2InterfaceOLinkClient::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	TWeakObjectPtr<UTbSame2SameStruct2InterfaceOLinkClient> weak(this);
...
void UTbSame2SameStruct2InterfaceOLinkClient::UseConnection(TScriptInterface<IApiGearConnection> InConnection)
{
	...
	{
		UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("The olink connection for the %s client does not implement the connection interface."), UTF8_TO_TCHAR(m_sink->olinkObjectName().c_str()));
		return;
	}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("ApiGear.TbSame2.SameStruct2Interface.OLink.Func1Async");
	if (!m_sink->IsReady())
	{
		UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("%s has no node. Probably no valid connection or service. Are the ApiGear TbSame2 plugin settings correct? Service set up correctly?"), UTF8_TO_TCHAR(m_sink->olinkObjectName().c_str()));
	...
	m_sink->GetNode()->invokeRemote(memberId, {Param1},
		...
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Func1Async: invalid return value type or null -> returning default"));
			Promise->SetValue(FTbSame2Struct1());
		}
	});
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE_STR("ApiGear.TbSame2.SameStruct2Interface.OLink.Func2Async");
	if (!m_sink->IsReady())
	{
		UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("%s has no node. Probably no valid connection or service. Are the ApiGear TbSame2 plugin settings correct? Service set up correctly?"), UTF8_TO_TCHAR(m_sink->olinkObjectName().c_str()));
	...
	m_sink->GetNode()->invokeRemote(memberId, {Param1, Param2},
		...
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Func2Async: invalid return value type or null -> returning default"));
			Promise->SetValue(FTbSame2Struct1());
		}
	});
...
void UTbSame2SameStruct2InterfaceOLinkClient::emitSignal(const std::string& signalName, const nlohmann::json& args)
{
	...
	if (signalName == "sig1")
	{
		...
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Signal sig1: invalid args array (expected 1 elements)"));
			return;
		}
		// make sure the type matches our expectation
		if (args[0].is_null() || !args[0].is_object())
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Signal param1: invalid type for parameter 0"));
			return;
		}
	...
	if (signalName == "sig2")
	{
		...
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Signal sig2: invalid args array (expected 2 elements)"));
			return;
		}
		// make sure the type matches our expectation
		if (args[0].is_null() || !args[0].is_object())
		...
		{
			UE_LOG(LogTbSame2SameStruct2InterfaceOLinkClient, Error, TEXT("Signal param2: invalid type for parameter 1"));
			return;
		}
