...
void UTbIfaceimportEmptyIfJniClientCache::init()
{
	...
	if (NewData->clientClassEmptyIf == nullptr || TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgCls))
	{
		return;
	}
	...
	if (NewData->clientClassEmptyIfCtor == nullptr || TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgInit))
	{
		return;
	}
	...
	if (NewData->BindMethodID == nullptr || TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgBind))
	{
		return;
	}
	...
	if (NewData->UnbindMethodID == nullptr || TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgUnbind))
	{
		return;
	}
...
bool UTbIfaceimportEmptyIfJniClient::_bindToService(FString servicePackage, FString connectionId)
{
	...
	if (MethodID != nullptr)
	{
		...
		if (TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgPackage))
		{
			return false;
		}
		...
		if (TbIfaceimportDataJavaConverter::CheckJniErrorOccurred(errorMsgId))
		{
			return false;
		}
...
void UTbIfaceimportEmptyIfJniClient::notifyIsReady(bool bInIsReady)
{
	b_isReady.store(bInIsReady, std::memory_order_release);

	auto broadcastConnectionStatusChanged = [](UTbIfaceimportEmptyIfJniClient& self)
	{
	...
	TWeakObjectPtr<UTbIfaceimportEmptyIfJniClient> weakSelf(this);
	AsyncTask(
		ENamedThreads::GameThread,
		[broadcastConnectionStatusChanged = MoveTemp(broadcastConnectionStatusChanged), weakSelf]
		{
		UTbIfaceimportEmptyIfJniClient* self = weakSelf.Get();
		if (self != nullptr)
		{
