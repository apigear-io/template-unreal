...
namespace Tests
{
BEGIN_DEFINE_SPEC(UTbIfaceimportEmptyIfOLinkSpec, "TbIfaceimport.EmptyIf.OLink", TbIfaceimportTestFilterMask);

TUniquePtr<FTbIfaceimportEmptyIfOLinkFixture> ImplFixture;

END_DEFINE_SPEC(UTbIfaceimportEmptyIfOLinkSpec);

void UTbIfaceimportEmptyIfOLinkSpec::Define()
{
	LatentBeforeEach([this](const FDoneDelegate& TestDone)
		{
		ImplFixture = MakeUnique<FTbIfaceimportEmptyIfOLinkFixture>();
		...
		// setup client
		UTbIfaceimportEmptyIfOLinkClient* OLinkClient = Cast<UTbIfaceimportEmptyIfOLinkClient>(ImplFixture->GetImplementation().GetObject());
