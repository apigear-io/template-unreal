...
{
	m_sink = std::make_shared<FOLinkSink>("tb.ifaceimport.EmptyIf");
}

UTbIfaceimportEmptyIfOLinkClient::UTbIfaceimportEmptyIfOLinkClient(FVTableHelper& Helper)
	: Super(Helper)
{
}
UTbIfaceimportEmptyIfOLinkClient::~UTbIfaceimportEmptyIfOLinkClient() = default;

void UTbIfaceimportEmptyIfOLinkClient::Initialize(FSubsystemCollectionBase& Collection)
{
	Super::Initialize(Collection);

	TWeakObjectPtr<UTbIfaceimportEmptyIfOLinkClient> weak(this);
...
void UTbIfaceimportEmptyIfOLinkClient::Deinitialize()
{
...
void UTbIfaceimportEmptyIfOLinkClient::UseConnection(TScriptInterface<IApiGearConnection> InConnection)
{
	if (!InConnection.GetInterface())
	{
		UE_LOG(LogTbIfaceimportEmptyIfOLinkClient, Error, TEXT("The olink connection for the %s client does not implement the connection interface."), UTF8_TO_TCHAR(m_sink->olinkObjectName().c_str()));
		return;
	}
...
bool UTbIfaceimportEmptyIfOLinkClient::_IsSubscribed() const
{
