...
#include "TbIfaceimportEmptyIfOLinkClient.generated.h"
DECLARE_LOG_CATEGORY_EXTERN(LogTbIfaceimportEmptyIfOLinkClient, Log, All);

UCLASS(NotBlueprintable, BlueprintType)
class TBIFACEIMPORTOLINK_API UTbIfaceimportEmptyIfOLinkClient : public UAbstractTbIfaceimportEmptyIf
{
	GENERATED_BODY()
public:
	explicit UTbIfaceimportEmptyIfOLinkClient();

	// only needed in 4.25 to use TUniquePtr<TbIfaceimportEmptyIfPropertiesData>
	UTbIfaceimportEmptyIfOLinkClient(FVTableHelper& Helper);
	virtual ~UTbIfaceimportEmptyIfOLinkClient();
	 ...
	 */
	UPROPERTY(BlueprintAssignable, Category = "ApiGear|TbIfaceimport|EmptyIf|Remote", DisplayName = "Subscription Status Changed")
