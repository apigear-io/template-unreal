...
namespace Tests
{
BEGIN_DEFINE_SPEC(UTestbed2NestedStruct1InterfaceMQTTSpec, "Testbed2.NestedStruct1Interface.MQTT", Testbed2TestFilterMask);

TUniquePtr<FTestbed2NestedStruct1InterfaceMQTTFixture> ImplFixture;

END_DEFINE_SPEC(UTestbed2NestedStruct1InterfaceMQTTSpec);

void UTestbed2NestedStruct1InterfaceMQTTSpec::Define()
{
	LatentBeforeEach([this](const FDoneDelegate& TestDone)
		{
		ImplFixture = MakeUnique<FTestbed2NestedStruct1InterfaceMQTTFixture>();
		TestTrue("Check for valid ImplFixture", ImplFixture.IsValid());

		TestTrue("Check for valid testImplementation", ImplFixture->GetImplementation().GetInterface() != nullptr);

		// setup client
		UTestbed2NestedStruct1InterfaceMQTTClient* MQTTClient = Cast<UTestbed2NestedStruct1InterfaceMQTTClient>(ImplFixture->GetImplementation().GetObject());
