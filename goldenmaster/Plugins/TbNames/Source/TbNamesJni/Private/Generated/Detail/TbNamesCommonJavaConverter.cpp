...
DEFINE_LOG_CATEGORY(LogTbNamesCommonJavaConverter_JNI);

bool FTbNamesCommonJavaConverter::CheckJniErrorOccurred(const TCHAR* Msg)
{
	...
	if (env->ExceptionCheck())
	{
		...
		env->ExceptionClear();
		UE_LOG(LogTbNamesCommonJavaConverter_JNI, Error, TEXT("%s"), Msg);
		return true;
	}
	return false;
}

bool FTbNamesCommonJavaConverter::TryFillArray(
    JNIEnv* Env, TArray<FString>& OutStringArray, jobjectArray InJniArray, const FString& InPropertyName)
{
	if (InJniArray == nullptr)
	{
		UE_LOG(LogTbNamesCommonJavaConverter_JNI, Warning, TEXT("%s is null"), *InPropertyName);
		return false;
	}
