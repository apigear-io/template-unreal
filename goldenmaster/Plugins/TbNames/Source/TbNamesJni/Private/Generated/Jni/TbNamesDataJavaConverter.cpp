...
TSharedPtr<FTbNamesDataJavaConverterCacheData, ESPMode::ThreadSafe> TbNamesDataJavaConverter::ensureInitialized()
{
	...
	if (CheckJniErrorOccurred(errorMsgEnumWithUnderScores))
	{
		UE_LOG(LogTbNamesDataJavaConverter_JNI, Warning, TEXT("TbNamesDataJavaConverter initialization failed"));
		return nullptr;
	}
	...
	if (CheckJniErrorOccurred(errorMsgNamEs))
	{
		UE_LOG(LogTbNamesDataJavaConverter_JNI, Warning, TEXT("TbNamesDataJavaConverter initialization failed"));
		return nullptr;
	}
