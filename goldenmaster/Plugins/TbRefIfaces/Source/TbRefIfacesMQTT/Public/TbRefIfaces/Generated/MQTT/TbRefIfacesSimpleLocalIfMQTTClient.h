...
#include "Misc/Guid.h"
#include "TbRefIfacesSimpleLocalIfMQTTClient.generated.h"

class TbRefIfacesSimpleLocalIfPropertiesData;
DECLARE_LOG_CATEGORY_EXTERN(LogTbRefIfacesSimpleLocalIfMQTTClient, Log, All);

UCLASS(NotBlueprintable, BlueprintType)
class TBREFIFACESMQTT_API UTbRefIfacesSimpleLocalIfMQTTClient : public UAbstractTbRefIfacesSimpleLocalIf
{
	GENERATED_BODY()
public:
	explicit UTbRefIfacesSimpleLocalIfMQTTClient();

	// only needed in 4.25 to use TUniquePtr<TbRefIfacesSimpleLocalIfPropertiesData>
	UTbRefIfacesSimpleLocalIfMQTTClient(FVTableHelper& Helper);
	virtual ~UTbRefIfacesSimpleLocalIfMQTTClient();
...
private:
	...
	/// @brief sets the value for the property IntProperty coming from the service
	/// @param InMsg contains the param of the type int32
	void setIntPropertyLocal(const IApiGearMqttClient::FReceivedMessage& InMsg);

	/// @brief publishes the value for the signal IntSignal coming from the service
	/// @param InMsg contains the param(s) of the type(s) int32 Param
	void onIntSignal(const IApiGearMqttClient::FReceivedMessage& InMsg);
