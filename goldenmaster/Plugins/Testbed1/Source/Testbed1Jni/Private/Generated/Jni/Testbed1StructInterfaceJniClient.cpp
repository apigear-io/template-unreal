...
void UTestbed1StructInterfaceJniClientCache::init()
{
	...
	if (NewData->clientClassStructInterface == nullptr || Testbed1DataJavaConverter::CheckJniErrorOccurred(errorMsgCls))
	{
		return;
	}
	...
	if (NewData->clientClassStructInterfaceCtor == nullptr || Testbed1DataJavaConverter::CheckJniErrorOccurred(errorMsgInit))
	{
		return;
	}
...
void UTestbed1StructInterfaceJniClient::Deinitialize()
{
	...
	gUTestbed1StructInterfaceJniClientmethodHelper.FlushPendingPromises();
...
void UTestbed1StructInterfaceJniClient::notifyIsReady(bool bInIsReady)
{
	b_isReady.store(bInIsReady, std::memory_order_release);

	auto broadcastConnectionStatusChanged = [](UTestbed1StructInterfaceJniClient& self)
	{
...
JNI_METHOD void Java_testbed1_testbed1jniclient_StructInterfaceJniClient_nativeAsyncOperationFailed(JNIEnv* Env, jclass Clazz, jstring callId, jstring errorMessage, jint errorCode)
{
	FString callIdString = FJavaHelper::FStringFromParam(Env, callId);
	static const TCHAR* errorMsgId = TEXT("failed to convert call id string in nativeAsyncOperationFailed for testbed1/testbed1jniclient/StructInterfaceJniClient");
	if (Testbed1DataJavaConverter::CheckJniErrorOccurred(errorMsgId))
	{
		return;
	}
	...
	UE_LOG(LogTestbed1StructInterfaceClient_JNI, Warning,
		TEXT("Async operation failed for id %s: %s (code: %d)"),
		*callIdString, *errorMsgString, static_cast<int32>(errorCode));

	FGuid guid;
	FGuid::Parse(callIdString, guid);
	gUTestbed1StructInterfaceJniClientmethodHelper.FulfillPromiseWithDefaultValue(guid);
}
#endif
