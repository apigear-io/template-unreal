...
void UTbSimpleEmptyInterfaceJniClient::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	if (TbSimpleJniCache::clientClassEmptyInterfaceCtor == nullptr)
	{
	...
	jobject localRef = Env->NewObject(TbSimpleJniCache::clientClassEmptyInterface, TbSimpleJniCache::clientClassEmptyInterfaceCtor);
...
bool UTbSimpleEmptyInterfaceJniClient::_bindToService(FString servicePackage, FString connectionId)
{
...
#if PLATFORM_ANDROID && USE_ANDROID_JNI
	JNIEnv* Env = FAndroidApplication::GetJavaEnv();
	if (TbSimpleJniCache::clientClassEmptyInterface == nullptr)
	{
	...
	jmethodID MethodID = TbSimpleJniCache::clientClassEmptyInterfaceBindMethodID;
	if (MethodID != nullptr)
	{
		jobject Activity = FJavaWrapper::GameActivityThis;
		auto jPackage = FJavaHelper::ToJavaString(Env, servicePackage);
		static const TCHAR* errorMsgPackage = TEXT("failed to craete java string for package in call bind on tbSimple/tbSimplejniclient/EmptyInterfaceJniClient");
		TbSimpleDataJavaConverter::checkJniError(errorMsgPackage);
		auto jConnId = FJavaHelper::ToJavaString(Env, connectionId);
		static const TCHAR* errorMsgId = TEXT("failed to craete java string for connection id in call bind on tbSimple/tbSimplejniclient/EmptyInterfaceJniClient");
...
void UTbSimpleEmptyInterfaceJniClient::_unbind()
{
	...
	jmethodID MethodID = TbSimpleJniCache::clientClassEmptyInterfaceUnbindMethodID;
	if (MethodID != nullptr)
	{
