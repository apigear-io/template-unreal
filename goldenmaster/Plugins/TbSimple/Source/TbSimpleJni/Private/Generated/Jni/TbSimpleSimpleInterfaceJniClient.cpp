...
void UTbSimpleSimpleInterfaceJniClient::notifyIsReady(bool bInIsReady)
{
	b_isReady.store(bInIsReady, std::memory_order_release);

	auto broadcastConnectionStatusChanged = [](UTbSimpleSimpleInterfaceJniClient& self)
	{
...
JNI_METHOD void Java_tbSimple_tbSimplejniclient_SimpleInterfaceJniClient_nativeAsyncOperationFailed(JNIEnv* Env, jclass Clazz, jstring callId, jstring errorMessage, jint errorCode)
{
	FString callIdString = FJavaHelper::FStringFromParam(Env, callId);
	static const TCHAR* errorMsgId = TEXT("failed to convert call id string in nativeAsyncOperationFailed for tbSimple/tbSimplejniclient/SimpleInterfaceJniClient");
	if (TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgId))
	{
		return;
	}
	...
	UE_LOG(LogTbSimpleSimpleInterfaceClient_JNI, Warning,
		TEXT("Async operation failed for id %s: %s (code: %d)"),
		*callIdString, *errorMsgString, static_cast<int32>(errorCode));

	FGuid guid;
	FGuid::Parse(callIdString, guid);
	gUTbSimpleSimpleInterfaceJniClientmethodHelper.FulfillPromiseWithDefaultValue(guid);
}
#endif
