...
void UTbSimpleNoPropertiesInterfaceJniClientCache::init()
{
	...
	if (NewData->clientClassNoPropertiesInterface == nullptr || TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgCls))
	{
		return;
	}
	...
	if (NewData->FuncVoidAsyncMethodID == nullptr || TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgFuncVoidAsyncMethod))
	{
		return;
	}
	...
	if (NewData->clientClassNoPropertiesInterfaceCtor == nullptr || TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgInit))
	{
		return;
	}
...
JNI_METHOD void Java_tbSimple_tbSimplejniclient_NoPropertiesInterfaceJniClient_nativeAsyncOperationFailed(JNIEnv* Env, jclass Clazz, jstring callId, jstring errorMessage, jint errorCode)
{
	FString callIdString = FJavaHelper::FStringFromParam(Env, callId);
	static const TCHAR* errorMsgId = TEXT("failed to convert call id string in nativeAsyncOperationFailed for tbSimple/tbSimplejniclient/NoPropertiesInterfaceJniClient");
	if (TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgId))
	{
		return;
	}
	...
	UE_LOG(LogTbSimpleNoPropertiesInterfaceClient_JNI, Warning,
		TEXT("Async operation failed for id %s: %s (code: %d)"),
		*callIdString, *errorMsgString, static_cast<int32>(errorCode));

	FGuid guid;
	FGuid::Parse(callIdString, guid);
	gUTbSimpleNoPropertiesInterfaceJniClientmethodHelper.FulfillPromiseWithDefaultValue(guid);
}
#endif
