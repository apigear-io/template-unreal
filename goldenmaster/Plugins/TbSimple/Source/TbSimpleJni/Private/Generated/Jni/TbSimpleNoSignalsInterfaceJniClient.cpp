...
void UTbSimpleNoSignalsInterfaceJniClientCache::init()
{
	...
	if (NewData->clientClassNoSignalsInterface == nullptr || TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgCls))
	{
		return;
	}
	...
	if (NewData->clientClassNoSignalsInterfaceCtor == nullptr || TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgInit))
	{
		return;
	}
...
void UTbSimpleNoSignalsInterfaceJniClient::OnPropBoolChanged(bool bInPropBool)
{
	...
	TWeakObjectPtr<UTbSimpleNoSignalsInterfaceJniClient> weakSelf(this);
	AsyncTask(
		ENamedThreads::GameThread,
		[updateAndBroadcastValueChanged = MoveTemp(updateAndBroadcastValueChanged), weakSelf]
		{
		UTbSimpleNoSignalsInterfaceJniClient* self = weakSelf.Get();
		if (self != nullptr)
		{
...
void UTbSimpleNoSignalsInterfaceJniClient::notifyIsReady(bool bInIsReady)
{
	b_isReady.store(bInIsReady, std::memory_order_release);

	auto broadcastConnectionStatusChanged = [](UTbSimpleNoSignalsInterfaceJniClient& self)
	{
...
JNI_METHOD void Java_tbSimple_tbSimplejniclient_NoSignalsInterfaceJniClient_nativeAsyncOperationFailed(JNIEnv* Env, jclass Clazz, jstring callId, jstring errorMessage, jint errorCode)
{
	FString callIdString = FJavaHelper::FStringFromParam(Env, callId);
	static const TCHAR* errorMsgId = TEXT("failed to convert call id string in nativeAsyncOperationFailed for tbSimple/tbSimplejniclient/NoSignalsInterfaceJniClient");
	if (TbSimpleDataJavaConverter::CheckJniErrorOccurred(errorMsgId))
	{
		return;
	}
	...
	UE_LOG(LogTbSimpleNoSignalsInterfaceClient_JNI, Warning,
		TEXT("Async operation failed for id %s: %s (code: %d)"),
		*callIdString, *errorMsgString, static_cast<int32>(errorCode));

	FGuid guid;
	FGuid::Parse(callIdString, guid);
	gUTbSimpleNoSignalsInterfaceJniClientmethodHelper.FulfillPromiseWithDefaultValue(guid);
}
#endif
