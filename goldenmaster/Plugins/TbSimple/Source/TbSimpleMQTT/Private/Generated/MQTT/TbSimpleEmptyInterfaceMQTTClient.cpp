...
DEFINE_LOG_CATEGORY(LogTbSimpleEmptyInterfaceMQTTClient);

UTbSimpleEmptyInterfaceMQTTClient::UTbSimpleEmptyInterfaceMQTTClient()
	: UAbstractTbSimpleEmptyInterface()
{
}

UTbSimpleEmptyInterfaceMQTTClient::UTbSimpleEmptyInterfaceMQTTClient(FVTableHelper& Helper)
	: Super(Helper)
{
}
UTbSimpleEmptyInterfaceMQTTClient::~UTbSimpleEmptyInterfaceMQTTClient() = default;

TMap<FString, ApiGear::MQTT::FCallbackFunction> UTbSimpleEmptyInterfaceMQTTClient::createTopicMap(const FString& ClientId)
{
	TMap<FString, ApiGear::MQTT::FCallbackFunction> Result;
	return Result;
}

void UTbSimpleEmptyInterfaceMQTTClient::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UTbSimpleEmptyInterfaceMQTTClient::Deinitialize()
{
...
void UTbSimpleEmptyInterfaceMQTTClient::UseConnection(TScriptInterface<IApiGearConnection> InConnection)
{
	if (!InConnection.GetInterface())
	{
		UE_LOG(LogTbSimpleEmptyInterfaceMQTTClient, Error, TEXT("The MQTT connection for the %s client does not implement the connection interface."), TEXT("TbSimpleEmptyInterface"));
		return;
	}
	...
	ApiGearMQTTClient->OnConnectionStatusChanged().AddUObject(this, &UTbSimpleEmptyInterfaceMQTTClient::onConnectionStatusChanged);
...
void UTbSimpleEmptyInterfaceMQTTClient::onConnectionStatusChanged(bool bIsConnected)
{
	if (!bIsConnected)
	{
	...
	TWeakObjectPtr<UTbSimpleEmptyInterfaceMQTTClient> weak(this);
...
void UTbSimpleEmptyInterfaceMQTTClient::DecrementPendingSubscription(bool bSuccess)
{
	if (!bSuccess)
	{
...
bool UTbSimpleEmptyInterfaceMQTTClient::_IsSubscribed() const
{
	return bIsSubscribed;
}

FGuid UTbSimpleEmptyInterfaceMQTTClient::RegisterResponseHandler(ApiGear::MQTT::InvokeReplyFunc&& Handler)
{
	FScopeLock Lock(&ResponseHandlerMutex);
...
void UTbSimpleEmptyInterfaceMQTTClient::onInvokeReply(const IApiGearMqttClient::FReceivedMessage& InMsg)
{
	if (InMsg.CorrelationData.Num() == 0)
	{
		UE_LOG(LogTbSimpleEmptyInterfaceMQTTClient, Error, TEXT("onInvokeReply: missing CorrelationData"));
		return;
	}
	...
	{
		UE_LOG(LogTbSimpleEmptyInterfaceMQTTClient, Error, TEXT("onInvokeReply: CorrelationData is not a valid GUID: %s"), *GuidStr);
		return;
	}
	...
	{
		UE_LOG(LogTbSimpleEmptyInterfaceMQTTClient, Warning, TEXT("onInvokeReply: no handler found for correlation id %s"), *ResponseId.ToString());
		return;
	}
