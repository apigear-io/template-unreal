...
namespace Tests
{
BEGIN_DEFINE_SPEC({{$Class}}MQTTSpec, "{{$ModuleName}}.{{$IfaceName}}.MQTT", {{$ModuleName}}TestFilterMask);

TUniquePtr<F{{$DisplayName}}MQTTFixture> ImplFixture;

END_DEFINE_SPEC({{$Class}}MQTTSpec);

void {{$Class}}MQTTSpec::Define()
{
	LatentBeforeEach([this](const FDoneDelegate& TestDone)
		{
		ImplFixture = MakeUnique<F{{$DisplayName}}MQTTFixture>();
		TestTrue("Check for valid ImplFixture", ImplFixture.IsValid());

		TestTrue("Check for valid testImplementation", ImplFixture->GetImplementation().GetInterface() != nullptr);

		// setup client
		U{{$DisplayName}}MQTTClient* MQTTClient = Cast<U{{$DisplayName}}MQTTClient>(ImplFixture->GetImplementation().GetObject());
