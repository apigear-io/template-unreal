...
#if PLATFORM_ANDROID && USE_ANDROID_JNI
		{{$Class}}Publisher* {{$Iface}}Publisher = ImplFixture->GetClient()->_GetPublisher();
		{{$Iface}}Publisher->On{{Camel .Name}}Changed.AddLambda([this, TestDone, count = 0]({{ueParam "In" .}}) mutable
#else
		{{$Class}}Publisher* {{$Iface}}Publisher = ImplFixture->GetLocalImplementation()->_GetPublisher();
		{{$Iface}}Publisher->On{{Camel .Name}}Changed.AddLambda([this, TestDone, StartValue, count = 0]({{ueParam "In" .}}) mutable
#endif
			{
