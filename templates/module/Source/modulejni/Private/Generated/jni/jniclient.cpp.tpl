...
{{- $javaClientPath := ( join "/" (strSlice ( camel $ModuleName) $jniclient_name) ) }}
	...
	if (NewData->{{$clientClass}} == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsgCls))
	{
		return;
	}
{{- range .Interface.Properties }}
{{- if not .IsReadOnly }}
	...
	if (NewData->{{ Camel .Name}}SetterId == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsg{{Camel .Name}}Setter))
	{
		return;
	}
{{- end }}
...
{{- range .Interface.Operations }}
	...
	if (NewData->{{Camel .Name}}AsyncMethodID == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsg{{Camel .Name}}AsyncMethod))
	{
		return;
	}
{{- end }}
	...
	if (NewData->{{$clientClass}}Ctor == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsgInit))
	{
		return;
	}
	...
	if (NewData->BindMethodID == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsgBind))
	{
		return;
	}
	...
	if (NewData->UnbindMethodID == nullptr || {{$localClassConverter}}::CheckJniErrorOccurred(errorMsgUnbind))
	{
		return;
	}
