...
{{- if .IsArray }}
    {{ jniToReturnType . }} {{snake .Name}}_value = (jobjectArray)env->GetObjectField(input, jFieldId_{{snake .Name}});		
{{- if (eq .KindType "enum") }}
...
{{- if (eq .KindType "enum") }}
    {{- $otherModuleClassName := printf "%sDataJavaConverter" ( Camel .Schema.Import ) }}
    jobjectArray {{$tmpObjName}} = s{{if not (eq $otherModuleClassName  "DataJavaConverter" )}}{{$otherModuleClassName}}::{{end -}}
...
{{- else if .IsPrimitive }}
    ...
    jFieldId_{{snake .Name}} = (*env)->New{{jniToEnvNameType .}}Array(len);
    if (jFieldId_{{snake .Name}}  == NULL){/*Log error, skip?*/};
    env->Set{{jniToEnvNameType .}}ArrayRegion(jFieldId_{{snake .Name}}, 0, len, {{$structName}}.{{$cppFieldName}}.GetData());
{{- else if not (eq .KindType "extern")}}
    {{- $otherModuleClassName := printf "%sDataJavaConverter" ( Camel .Schema.Import ) }}
