...
This document only explains the bare minimum needed to know to get from an API description to a functional *Unreal Engine* plugin.
...
In case you want to check or modify the source code of the template, it is easier to clone or download the repository.
```bash
...
### API module file
Create the `helloworld.module.yaml` with the example content:
...
📦ue_docs_example_project
 ...
 ┃ ┗ 📜ue_docs.uproject
```
With the solution file from the previous paragraph the code will be generated in the `ue_docs/Plugins` folder.
### Generate via CLI
...
In *Blueprint* we first create an object of the type *Io World Hello*. This can then be  directly used or stored in a variable.
...
Since the generated interface is probably used outside of its own module, we have to add the module identifier to the list of dependency modules in the dependent module e.g. your project's or another plugin's module.
This can be done like this, in the example *IoWorld* is the demo module's name:
...
The following code is an example and should not be used like this in production code.
